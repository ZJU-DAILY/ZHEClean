...
class OTE(nn.Module):
    ...
    def orth_embedding(self, embeddings, eps=1e-18, do_test=True):
        ...
        for i in range(1, self.num_elem):
            u_d = u_d - u[-1].unsqueeze(dim=1) * (
                        (embeddings[:, i:] * u[i - 1].unsqueeze(dim=1)).sum(dim=-1) / uu[i - 1].unsqueeze(
                    dim=1)).unsqueeze(-1)
            u_i = u_d[:, 0]
            u_d = u_d[:, 1:]
    ...
    def orth_reverse_mat(self, rel_embeddings):
        ...
        if self.use_scale:
            ...
            rel_embeddings = rel_embeddings.view(-1, self.num_elem, self.num_elem).transpose(1, 2).contiguous().view(
                rel_size)
